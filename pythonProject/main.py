...
def get_cards(conn, set_id):
    ...
    rows = cursor.fetchall()
    cards = {row[0]: row[1] for row in rows}  # Create a dictionary of cards (word: definition)
    return cards
...
def show_card():
    ...
    if current_tabs:
        if 0 <= card_index < len(current_tabs):
            word,_ = current_tabs[card_index]
...
def next_card():
    ...
    global current_tabs
    if current_tabs:
        if card_index < len(current_tabs) - 1:
            card_index+=1
        show_card()
...
if __name__ == '__main__':
    ...
    ttk.Label(create_set_frame, text='Front:').pack(padx=5, pady=5)
    ttk.Entry(create_set_frame, textvariable=word_var, width=30).pack(padx=5, pady=5)

    ttk.Label(create_set_frame, text='Back:').pack(padx=5, pady=5)
    ...
    ttk.Button(select_set_frame, text='Delete Set', command=delete_selected_sets).pack(padx=5, pady=5)
    ...
    word_label = ttk.Label(flashcards_frame, text='', font='TkDefaultFont')
